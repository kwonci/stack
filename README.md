...
- **`/data` 를 키우려면 `DATA_VOLUME_SIZE` 를 올리고 `make down && make up`.** 볼륨은 그 자리에서 커지지만 파일시스템은 cloud-init 의 `resize2fs` 가 늘려 주고, 그건 인스턴스가 새로 만들어질 때만 돕니다(재부팅으로는 안 됩니다). 인스턴스를 유지한 채 늘리려면 `make ssh ARGS='sudo resize2fs $(findmnt -no SOURCE /data)'`.
- **줄이는 것은 그 자리에서 안 됩니다.** EBS 의 `ModifyVolume` 은 확대만 받으므로 `DATA_VOLUME_SIZE` 를 낮추면 apply 가 실패할 뿐 데이터는 안전합니다. 실제로 줄이려면 볼륨을 새로 만들어 갈아끼워야 합니다: `make down` → `make destroy-data` → `.env` 의 크기를 낮춤 → `make up`. **이 경로는 데이터를 버립니다.** `destroy-data` 가 남기는 `final_snapshot` 은 줄이기 전 크기의 스냅샷이라, 더 작은 볼륨으로는 복원되지 않기 때문입니다(EBS 는 스냅샷보다 작은 볼륨을 거부합니다). 내용을 지키려면 줄이기 전에 호스트에서 `/data` 를 따로 빼내 두었다가 새 볼륨에 되돌려 놓으세요.
