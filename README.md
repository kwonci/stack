...
세 번째 줄이 핵심입니다. Cloudflare 엣지는 AWS 서비스가 아니라 VPC 엔드포인트가 존재하지 않으므로, NAT 없이 사설 서브넷에 두면 터널 자체가 성립하지 않습니다. 스택 전체가 월 $6.5 인데 NAT 하나가 그 7배입니다.
...
`make down` 상태에서도 남는 것 (ap-northeast-2 기준, 대략):

| 리소스 | 월 |
|---|---|
| EBS 데이터 볼륨 30GB gp3 | ~$2.7 |
| 공인 IPv4 (자동 할당) | ~$3.6 |
| S3 tfstate | ~$0 |
